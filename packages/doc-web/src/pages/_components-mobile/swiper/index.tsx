...
function Swiper() {
  ...
  const totalGroup = [...group, ...group, ...group, ...group];
  const list = [[], [], [], []];
    ...
    <div className={clsx(styles.swiper)}>
      {list.map((el, idx) => (
        <div className={clsx(styles.container)} key={el[index].name}>
          <div className={clsx(styles['swiper-item'], styles.animation, styles[`animation-${idx + 1}-${tag}`])}>
            <img className={clsx(styles.img)} src={el[index].photo} alt="" loading="lazy" />
