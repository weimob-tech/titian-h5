...
| 名称     | 参数列表 | 描述           | 备注 |
| -------- | -------- | -------------- | ---- |
| onchange | `(e: Event) => void`    | 切换菜单时触发, 返回激活项索引| -    |
| onscrolltoupper | `(e: Event<{ direction: 'top' }>) => void`    | 滚动到顶部时触发| -    |
| onscrolltolower | `(e: Event<{ direction: 'bottom' }>) => void`    | 滚动到底部时触发| -    |
| onscroll | `(e: Event) => void`    | 滚动时触发| -    |
