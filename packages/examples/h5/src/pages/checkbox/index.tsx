...
const Checkbox: React.FC<Record<string, never>> = () => {
  ...
  const Comp = attrs.type === 'TiCheckboxButton' ? 'ti-checkbox-button' : 'ti-checkbox';

  return (
    <div style={style}>
      <Page pageClass="checkbox-button-page" options={options} change={change}>
        <ti-checkbox-group max={attrs.limit} defaultValue={[0]} color={attrs.color} icon={attrs.iconName}>
          {attrs.group?.map((item, index) => (
            <Comp label={item} disabled={attrs.disabled} value={index} key={index}>
              {attrs.ext === 'image' && (
                <ti-image
                  ext-style={`margin-right: ${addUnit(12)}`}
                  slot="prefix"
                  width="40"
                  radius="4"
                  height="40"
